...
class Expr {
 public:
  ...
  virtual double Evaluate(const ValueGetter &value_getter) const = 0;
...
// При делении на 0 выбрасывайте ошибку вычисления FormulaError
  double Evaluate(const ValueGetter &value_getter) const override {
    double lhs_value = lhs_->Evaluate(value_getter);
    double rhs_value = rhs_->Evaluate(value_getter);
    double result;
...
// Реализуйте метод Evaluate() для унарных операций.
  double Evaluate(const ValueGetter &value_getter) const override {
    switch (type_) {
      case UnaryPlus: {
        return +operand_->Evaluate(value_getter);
      }
      case UnaryMinus: {
        return -operand_->Evaluate(value_getter);
      }
...
double FormulaAST::Execute(const ValueGetter &value_getter) const {
  return root_expr_->Evaluate(value_getter);
}
